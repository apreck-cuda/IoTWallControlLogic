    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if ioState == 1:
                ...
                print("Rule activated with code: " + str(responseRule))
                ioControl("red", "1")
                urlRuleAccess = "http://" + fwIP + ":8080/rest/firewall/v1/forwarding-firewall/rules/dynamic/" + dynRule
                payloadAccess = {
                   "action": "disable",
                   "expiresIn": 0,
                   "expireAction": "disable"
                }
                responseRuleAccess = requests.post( urlRuleAccess, json=payloadAccess, auth=HTTPBasicAuth(username, password), timeout=5)
                print("Remote Access deactivated with code: " + str(responseRuleAccess))
                time.sleep(2)
            else:
                print("No Emergancy STOP detected - all good")
                ioControl("red", "0")
                ioControl("green", "1")
                print("Deaktivating Emergany Rule")
                urlRule = "http://" + fwIP + ":8080/rest/firewall/v1/forwarding-firewall/rules/dynamic/" + stopRule
                username = "api"
                password = "Demo-123"
                payload = {
                ...
                responseRule = requests.post( urlRule, json=payload, auth=HTTPBasicAuth(username, password), timeout=5)
                print("Rule deaktivated with code: " + str(responseRule))
        else:
            print("Request failed with status code:", response.status_code)
    except requests.exceptions.Timeout:
        print("Request timed out")
...
def ipsHit():
    ...
    try:
        if int(ipsCount) >= 1:
            ...
            responseRule = requests.post( urlRule, json=payload, auth=HTTPBasicAuth(username, password), timeout=5)
            print("Remote Access deactivated with code: " + str(responseRule))
        else:
